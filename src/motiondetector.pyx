...
class SlidingWindow:
    def __init__(self, int shape, dtype):
    ...
    def get_frames(self):
        if self.last_index is None:
            return []
        frames = []
        cdef int cur = self.oldest_index
        cdef int end_index = (self.last_index + 1) % self.size
    ...
    def add(self, frame, ffc=False):
        ...
        if self.last_index is None:
            ...
            if new_index == self.oldest_index:
                if self.oldest_index == self.non_ffc_index and not _ffc:
...
class WeightedBackground:
    ...
    def process_frame(self, cptv_frame, bint ffc_affected):
        cdef bint prev_ffc, back_changed
        cdef int res_x=160
        cdef int res_y=120
        self.num_frames += 1
        ...
        cdef cnp.ndarray frame = np.int32(self.crop_rectangle.subimage(self.running_mean.mean()))
        ...
        back_changed = np.any(new_background != edgeless_back)
        if back_changed:
