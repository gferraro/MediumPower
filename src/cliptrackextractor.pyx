...
class ClipTrackExtractor:
    ...
    def _get_filtered_frame(self, cnp.ndarray thermal, bint sub_change=True, bint denoise=False):
        """
    ...
    def process_frame(self, clip, frame, **args):
        ...
        cdef bint ffc_affected = is_affected_by_ffc(frame)
    ...
    def _get_regions_of_interest(self, clip, component_details, centroids=None):
        ...
        cdef int i, padding = self.frame_padding
        cdef double crop_width_fraction, crop_height_fraction
        cdef int extra_edge

        delta_filtered = self.get_delta_frame(clip)
